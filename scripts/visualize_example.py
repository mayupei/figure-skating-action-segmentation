...
from file_paths import RAW_DATA_PATH, OUTPUT_PATH
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run skeleton animation and save it to a file.")
    parser.add_argument('--input', type=str, required=True,
                        help='file name')
    parser.add_argument('--split', type=int, required=True,
                        help='integer starting from 0')
    return parser.parse_args()

def create_video_and_save(file_name, split):
    preds = np.load(os.path.join(OUTPUT_PATH, "stage2", f"{file_name}_{split}.npy"))
    labels = np.load(os.path.join(OUTPUT_PATH, "ground_truth", f"{file_name}_{split}.npy"))
    ...
    create_video_with_predictions(skeletons, preds_full, labels_full, f"{file_name}_{split}")
